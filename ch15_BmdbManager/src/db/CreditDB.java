...
public class CreditDB extends BaseDB implements DAO<Credit> {
	...
	public Credit get(int id) {
		Credit credit = null;
		String sql = "select * from credit c" + "  join actor on actorid = actor.ID"
				+ "  join movie on movieid = movie.ID" + "  where c.id = ?";
	...
	public List<Credit> getAll() {
		...
		try (Statement stmt = getConnection().createStatement()) {
			ResultSet rs = stmt.executeQuery("select * from credit" + "  join actor on actorid = actor.ID"
	...
	public List<Credit> getCreditsForMovie(Movie movie) {
		List<Credit> credits = new ArrayList<>();
		String sql = "select * from credit" + "  join actor on actorid = actor.ID"
				+ "  join movie on movieid = movie.ID" + "  where movieid = ?";
		try (PreparedStatement stmt = getConnection().prepareStatement(sql)) {
